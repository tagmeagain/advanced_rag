...
class MarkdownChunker:
    ...
    def process_dataframe(self, df: pd.DataFrame, content_column: str = 'filecontent') -> pd.DataFrame:
        """
        Process a DataFrame containing markdown content and add chunking information.
        
        Args:
            df: DataFrame containing markdown content
            content_column: Name of the column containing markdown content
            
        Returns:
            DataFrame with added chunkid and chunk_text columns
        """
        # Create new lists to store chunk information
        chunk_ids = []
        chunk_texts = []
        original_indices = []
        ...
        for idx, row in df.iterrows():
            ...
            # Add each chunk to our lists
            for chunk in chunks:
                chunk_ids.append(chunk.chunk_id)
                chunk_texts.append(chunk.content)
                original_indices.append(idx)
        
        # Create a new DataFrame with the chunk information
        ...
        result_df = pd.concat([
            df.iloc[original_indices].reset_index(drop=True),
            chunk_df
        ], axis=1)
